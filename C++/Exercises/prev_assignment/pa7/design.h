...
class Design {
 public:
  ...
  string name(); // returns name of Design
  void addPI(string n); // adds primary input
  void addPO(string n); // adds primary output
 ...
 private:
  ...
  map<string, int> colorMap; // map containing colors assigned to string names
  vector<string> pis;
  vector<string> pos;
  // pointer to a vector storing pointers to Net
  vector<Net *> sortedNodes; // for part 2
};
